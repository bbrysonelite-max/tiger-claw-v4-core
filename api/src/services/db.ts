...
/** Return the bot_token assigned to a tenant, or null. BYOB — token is on the tenant record. */
export async function getTenantBotToken(tenantId: string): Promise<string | null> {
  ...
  return tRes.rows[0]?.bot_token as string ?? null;
}

/** Return the bot username assigned to a tenant, or null. BYOB — username is on the tenant record. */
export async function getTenantBotUsername(tenantId: string): Promise<string | null> {
  ...
  return tRes.rows[0]?.bot_username as string ?? null;
}
