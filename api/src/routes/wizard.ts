...
import { generateMagicToken, verifyMagicToken } from "../services/email.js";
import { z } from "zod";
...
// Requires a valid HMAC-SHA256 token + expiry generated by sendStanStoreWelcome.
// Admin-generated links (GET /admin/magic-link) produce the same signed format.
router.get("/auth", async (req: Request, res: Response) => {
  ...
  // Validate HMAC token when present. Reject if token/expires are missing or invalid.
  if (!token || !expires) {
    return res.status(401).json({ error: "Missing magic link token. Request a new link." });
  }
  if (!verifyMagicToken(email, token, Number(expires))) {
    return res.status(401).json({ error: "Invalid or expired magic link. Request a new link." });
  }
